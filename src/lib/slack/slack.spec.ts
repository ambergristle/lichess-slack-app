...
import { generateAuthorizationUrl, validateRegistrationRequest, verifySlackSignature } from '.'
...
describe('Verify Slack Signature', () => {
  const app = new Hono()
    ...
    .get('/', (c) => c.text('ok'))
    .onError((error, c) => {
      const status = error instanceof KnownError
        ? error.status
        : 500
...
describe('Generate Authorization URL', () => {
  ...
  test('', async () => {
    ...
    const [value, ageLimit, path, httpOnly, sameSite] = stateCookie!.split('; ')
